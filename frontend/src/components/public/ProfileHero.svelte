...
<header class="grain relative bg-white dark:bg-gray-900 text-gray-900 dark:text-white" itemscope itemtype="https://schema.org/Person">
	<div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 {heroSpacingClass}">
		...
		{#if profile?.headline}
			<p class="text-2xl sm:text-3xl text-gray-600 dark:text-gray-300 mb-6 leading-relaxed" itemprop="jobTitle">
				{profile.headline}
			</p>
		{/if}

		{#if profile?.location}
			<p class="flex items-center gap-2 text-gray-600 dark:text-gray-300 mb-8" itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
				<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
				</svg>
