<script lang="ts">
	...
	let smtpConfigured = $state(true); // optimistic default

	// Track saved state for navigation guard
	let lastSavedSubject = $state('');
	let lastSavedBody = $state('');

	// Polling state for in-progress sends
	let pollTimer: ReturnType<typeof setTimeout> | null = null;
	let pollInterval = 2000;
	const POLL_MIN = 2000;
	const POLL_MAX = 10000;

	// Derived unsaved content flag
	let hasUnsavedContent = $derived(
		subject !== lastSavedSubject || bodyMarkdown !== lastSavedBody
	);

	// Sanitize preview HTML with DOMPurify
	$effect(() => {
		if (bodyMarkdown) {
	...
	// Navigation guard — warn before leaving with unsaved content
	...
	onMount(async () => {
		...
		await Promise.all([loadSends(), loadTemplates(), checkSmtpStatus()]);
		startPollingIfNeeded();
	});

	async function checkSmtpStatus() {
		try {
			const resp = await fetch('/api/admin/newsletter/smtp-status', {
	...
	async function loadTemplates() {
		try {
			const resp = await fetch('/api/admin/newsletter/templates', {
				headers: pb.authStore.isValid ? { Authorization: `Bearer ${pb.authStore.token}` } : {}
			});
			if (resp.ok) {
	...
	// Exponential backoff polling for send status
	...
	async function sendToAll() {
		...
		// Pre-fetch active subscriber count
		let activeCount = 0;
		try {
		...
		const confirmed = await confirm({
			...
			message: $t('admin.newsletter.send_confirm_message_count', { values: { count: activeCount } }),
		...
		try {
			...
			// Reset saved state so navigation guard doesn't trigger
			subject = '';
			bodyMarkdown = '';
			lastSavedSubject = '';
			lastSavedBody = '';
...
<div class="max-w-5xl mx-auto space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<a href="/admin/subscribers" class="btn btn-ghost">
			{$t('admin.newsletter.back_to_subscribers')}
		</a>
	</div>
	...
	{#if templates.length > 0}
		<div class="card p-4">
			...
			{#if selectedTemplate}
				{@const activeTpl = templates.find((t) => t.slug === selectedTemplate)}
				{#if activeTpl}
	...
	<div class="card p-6 space-y-4">
		<div>
			<label for="newsletter-subject" class="label text-xs"
				>{$t('admin.newsletter.subject_label')} *</label
			>
			<input
				type="text"
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
			<div>
				<label for="newsletter-body" class="label text-xs"
					>{$t('admin.newsletter.body_label')} *</label
				>
				<textarea
					id="newsletter-body"
				...
				<table class="w-full text-sm" aria-label={$t('admin.newsletter.send_history')}>
					...
					<tbody>
						{#each sends as send (send.id)}
							...
							>
								<td class="py-3 px-4 text-gray-900 dark:text-white font-medium">{send.subject}</td>
								<td class="py-3 px-4">
									<span
								...
								<td class="py-3 px-4 text-gray-600 dark:text-gray-400">
									{#if send.status === 'sent' && send.recipient_count > 0}
										<span class="inline-flex items-center gap-2 text-xs">
											<span title="Opens">
												{send.open_count || 0}
												{$t('admin.newsletter.opens', { values: { count: send.open_count || 0 } })}
											...
											<span title="Clicks">
												{send.click_count || 0}
												{$t('admin.newsletter.clicks', { values: { count: send.click_count || 0 } })}
											</span>
										</span>
									{:else}
