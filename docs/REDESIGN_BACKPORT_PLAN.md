...
| B3 | Default font pack → Hanken/Newsreader (keep packs selectable) | med-high | **DONE** — PR #452 (stacked on #450). 'soft-premium' font pack injected inline via pipeline; classic loads nothing extra. soft-premium-fonts.spec.ts. |
| B4 | Default accent → terracotta (code default only) | low | TODO |
| B5 | `grain`/`font-accent`/`text-gradient`/`divider-editorial` utilities | med | **DONE** — PR #453 (stacked on #452). Authored unlayered to survive Tailwind purge. soft-premium-utils.spec.ts. |
...
- 2026-06-23 (resumed) — **B3 DONE** (PR #452, `feat/soft-premium-fonts` stacked on #450): added a 'soft-premium' font pack (Hanken Grotesk + Newsreader), default under soft-premium unless operator picked a pack. Key insight: fonts (like accent) are injected INLINE by hooks.server, which beats `:root[data-design]` rules — so the font swap goes through the pack pipeline, not a CSS override. Conditional load = zero classic cost. Product-tested. **Visual foundation B0-B3 now complete + certified.** Remaining: B4 (terracotta default accent — small; needs SSR+client default plumbing), B5 (grain/font-accent/text-gradient/divider utils — low value until Track C consumes them), **Track C (10 surface ports — the bulk)**, A2/A9/A11. Stacking chain: B0(#448)<-B1(#450)<-B3(#452)<-[B4/B5]<-Track C; B2(#451)+TrackA(#449) independent. Consider merging the green chain to main to simplify Track C.
